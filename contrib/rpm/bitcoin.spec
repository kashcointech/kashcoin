...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 17882
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 17888
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 27882
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 27888
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 17882
	%{_sbindir}/semanage port -d -p tcp 17888
	%{_sbindir}/semanage port -d -p tcp 27882
	%{_sbindir}/semanage port -d -p tcp 27888
	for selinuxvariant in %{selinux_variants}; do
