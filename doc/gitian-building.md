...
*Setup instructions for a Gitian build of Kashcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Kashcoin
...
- [Building Kashcoin Core](#building-kashcoin-core)
...
Clone the git repositories for kashcoin and Gitian.
...
in the kashcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Kashcoin Core
----------------

To build Kashcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the kashcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/kashcoin/.git/
...
For example:
```bash
URL=https://github.com/thrasher-/kashcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit kashcoin=${COMMIT} --url kashcoin=${URL} ../kashcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit kashcoin=${COMMIT} --url kashcoin=${URL} ../kashcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit kashcoin=${COMMIT} --url kashcoin=${URL} ../kashcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the kashcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../kashcoin/contrib/gitian-descriptors/*|sort|uniq )
...
./bin/gbuild --url kashcoin=${BTCPATH},signature=${SIGPATH} ../kashcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/kashcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/kashcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/kashcoin-osx-build.assert
```
